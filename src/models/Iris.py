...
class Iris(Node):
    def __init__(self, name='') -> None:
        ...
        self.declare_parameter('hoverThrottle', 0.0)
    ...
    def attitudeCallback(self, msg):
        ...
        if self.useCamera:
            self.qBuffer.addMessage(self.meQuaternionNED)
            qDelay = self.qBuffer.getMessage()
            if qDelay != None:
                print('With camera, using delay IMU information with qBuffer......')
                self.meRPYRadNEDDelay = quaternion2euler(qDelay)
                self.meRPYRadENUDelay = rpyNED2ENU(self.meRPYRadNEDDelay)
